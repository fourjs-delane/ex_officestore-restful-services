...
# 
# Four Js and its products do not warrant or guarantee that these samples
...
FUNCTION getRecords()
    ...
    CALL logger.logEvent(logger._LOGDEBUG ,SFMT("product:%1",__LINE__),"getRecords",
...
PRIVATE FUNCTION updateRecordById(thisData STRING) RETURNS INTEGER
    ...
    EXECUTE recordUpdate USING thisRecord.prodname, thisRecord.proddesc, thisRecord.productid -- a list from the record thisData.prodid
...
#+
#+ @code 
#+ IF product.isValidQuery(queryParameter)
#+
#+ @param 
...
PUBLIC FUNCTION addQueryFilter(columnKey STRING, columnValue STRING)
    ...
    CASE (columnKey)
        WHEN "prodnum"
            LET mSqlWhere = SFMT("%1 %2 '%3'", mSqlWhere, " AND productid LIKE ", columnValue)
    END CASE
