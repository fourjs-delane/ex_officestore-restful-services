...
#+
#+ @code
#+ CALL orderItem.getJSONEncoding()
#+
#+ @param 
#+ NONE
#+
...
#+
#+ Description: Perform the tasks to update the changes to the sample records for storage
#+    to the data source
#+
#+ @code
...
#+
#+ Description: Executes the update to the given sample record in the datasource returning  
...
PRIVATE FUNCTION updateRecordById(thisData STRING) RETURNS INTEGER
    ...
    --    PREPARE recordUpdate FROM _UPDATESQL
    EXECUTE recordUpdate USING thisRecord.orderitem.unitprice,   -- a list from the record thisData.orderid and linenum
                               thisRecord.orderitem.orderid,
                               thisRecord.orderitem.linenum    
...
#+
#+ Description: Executes the insert into the datasource returning success/fail 
#+
#+ @code
...
#+ Method: insertRecord
#+
#+ Description: Executes the insert of the given sample record into the datasource returning  
#+    the number of rows processed.  If no rows found, return zero.
#+
#+ @code
...
--  PREPARE recordDelete FROM _DELETESQL
    EXECUTE recordDelete USING thisRecord.orderitem.orderid, thisRecord.orderitem.linenum
...
PUBLIC FUNCTION addQueryFilter(columnKey STRING, columnValue STRING)
    ...
    CASE (columnKey)
        WHEN "order"
            LET mSqlWhere = SFMT("%1 %2 '%3'", mSqlWhere, " AND orderid LIKE ", columnValue)
    END CASE
...
FUNCTION initQuery()
    ...
    LET mSqlWhere = "WHERE a.itemid = b.itemid AND b.productid = c.productid"
END FUNCTION
