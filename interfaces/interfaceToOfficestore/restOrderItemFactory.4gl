...
FUNCTION processQuery(requestPayload STRING) RETURNS (INTEGER, STRING)
    ...
    TRY
        ...
        CALL logger.logEvent(logger._LOGDEBUG ,SFMT("orderItemFactory:%1",__LINE__),"processQuery",
        ...
        FOR i = 1 TO query.getLength()
            ...
            IF orderItem.isValidQuery(query[i].keyName) THEN
                CALL orderItem.addQueryFilter(query[i].keyName, query[i].keyValue)
            ELSE 
        ...
        IF NOT queryException THEN
            ...
            # Set response data
            CALL interfaceRequest.setResponse(HTTP_OK, "SUCCESS", "", orderItem.getJSONEncoding())
        END IF
