...
FUNCTION processQuery(queryFilter STRING) RETURNS (INTEGER, STRING)
    ...
    TRY
        ...
        CALL logger.logEvent(logger._LOGDEBUG ,SFMT("supplierFactory:%1",__LINE__),"processQuery",
        ...
        FOR i = 1 TO query.getLength()
            ...
            IF supplier.isValidQuery(query[i].name) THEN
                CALL supplier.addQueryFilter(query[i].name, query[i].value)
            ELSE 
